...
const realUsers = db
  .prepare("SELECT count(*) c FROM users WHERE google_sub NOT IN ('smoke-fake-sub','viewer-fake-sub')")
  .get();
if (realUsers.c > 0) {
  console.error(
    "REFUSING TO SEED: this database has real (non-demo) users. Demo seed only works on an empty/demo database.",
  );
  process.exit(1);
}
const demoIds = db
  .prepare("SELECT id FROM users WHERE google_sub IN ('smoke-fake-sub','viewer-fake-sub')")
  .all()
  .map((r) => r.id);
if (demoIds.length > 0) {
  const ph = demoIds.map(() => "?").join(",");
  const demoSheets = db
    .prepare(`SELECT id FROM spreadsheets WHERE user_id IN (${ph})`)
    .all(...demoIds)
    .map((r) => r.id);
  if (demoSheets.length > 0) {
    const ph2 = demoSheets.map(() => "?").join(",");
    const demoTabs = db
      .prepare(`SELECT id FROM tabs WHERE spreadsheet_id IN (${ph2})`)
      .all(...demoSheets)
      .map((r) => r.id);
...
db.prepare(
  ...
  userId,
  "fakeGoogleId123",
  "US2 Daily Production",
  "https://docs.google.com/spreadsheets/d/fakeGoogleId123/edit",
  "off",
  now - 1 * HOUR,
  now - 48 * HOUR,
);
...
db.prepare("INSERT INTO tabs (id, spreadsheet_id, title, position, tracked, key_column) VALUES (?,?,?,0,1,0)").run(
  tabPE4,
  sheetId,
  "PE4",
);
db.prepare("INSERT INTO tabs (id, spreadsheet_id, title, position, tracked, key_column) VALUES (?,?,?,1,1,0)").run(
  tabPE7,
  sheetId,
  "PE7",
);
...
db.prepare("INSERT INTO members (id, owner_user_id, email, created_at) VALUES (?,?,?,?)").run(
  crypto.randomUUID(),
  userId,
  "viewer@test.local",
  now,
