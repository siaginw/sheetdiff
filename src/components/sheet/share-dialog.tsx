  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <ShareBody members={members} />
      </DialogContent>
    </Dialog>
  );
}

/** State lives here, INSIDE the portal content: Base UI unmounts it when the
 *  dialog closes, so every open starts fresh from props — the fix for the
 *  stale-banner/programmatic-open class (controlled dialogs never fire
 *  onOpenChange when the parent calls setOpen(true)). */
function ShareBody({ members }: { members: { id: string; email: string }[] }) {
