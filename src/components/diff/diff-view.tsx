...
import { ackAllUnentered, toggleAck } from "@/lib/actions";
import { DiffRow, oldRowValues, type DiffResult } from "@/lib/diff/engine";
import { columnWidths } from "@/lib/diff/widths";
import { shouldWordDiff, wordDiff } from "@/lib/diff/worddiff";
import { useVirtualizer } from "@tanstack/react-virtual";
import { ArrowUpDown, Check, CheckCircle2, Minus, PenLine, Plus, Search } from "lucide-react";
...
function DiffStat({ s }: { s: DiffResult["summary"] }) {
  return (
    <span className="flex flex-wrap items-center gap-x-3 gap-y-1 font-mono text-xs">
      {s.addedRows > 0 && (
        <span
          className="font-medium text-diff-add-fg"
          title={`${s.addedRows} added ${s.addedRows === 1 ? "row" : "rows"}`}
        >
      ...
      {s.removedRows > 0 && (
        <span
          className="font-medium text-diff-del-fg"
          title={`${s.removedRows} removed ${s.removedRows === 1 ? "row" : "rows"}`}
        >
      ...
      {s.changedRows > 0 && (
        <span
          className="font-medium text-diff-move-fg"
          title={`${s.changedCells} changed ${s.changedCells === 1 ? "cell" : "cells"} in ${s.changedRows} ${s.changedRows === 1 ? "row" : "rows"}`}
        >
      ...
      {s.movedRows > 0 && (
        <span
          ...
          title={`${s.movedRows} ${s.movedRows === 1 ? "row" : "rows"} moved position`}
        >
          →{s.movedRows} moved
        </span>
      )}
      {s.columnsAdded.map((c) => (
        <span key={`+${c}`} className="text-diff-add-fg">
          +col “{c}”
        </span>
      ))}
      {s.columnsRemoved.map((c) => (
        <span key={`-${c}`} className="text-diff-del-fg">
          −col “{c}”
        </span>
      ))}
      ...
      <form action={toggleAck}>
        ...
        <button
          ...
          className={`rounded-md p-2 transition-colors ${
            acked ? "text-diff-add-fg" : "text-muted-foreground/50 hover:bg-muted hover:text-foreground"
          }`}
        >
          <Check className={`size-3.5 ${acked ? "fill-current" : ""}`} />
        </button>
      </form>
      <NoteDialog spreadsheetId={spreadsheetId} tabId={tabId} rowKey={row.rowKey} existingNote={note} />
    </span>
  );
}
    ...
    <>
      {wordDiff(from, to).map((seg, i) =>
        seg.kind === "same" ? (
          ...
          <span key={i} className="rounded-sm bg-diff-del-token px-0.5 line-through decoration-diff-del-fg/60">
            {seg.text}
          </span>
        ) : (
          <span key={i} className="rounded-sm bg-diff-add-token px-0.5 font-semibold">
            {seg.text}
          </span>
        ),
      )}
  ...
  for (const r of rows) {
    ...
    if (r.status === "changed") {
      out.push({
        ...
        sign: "-",
        row: r,
        changedCols: new Set(r.cells.map((c) => c.col)),
        id: `del-${r.newIndex}-${out.length}`,
      });
      out.push({
        kind: "sign",
        sign: "+",
...
function CodeLine({
  ...
  if (item.kind === "gap") {
    return (
      <div className="flex h-8 items-center gap-2 border-y border-diff-hunk-bg bg-diff-hunk-bg/50 px-3 font-mono text-[11px] text-diff-hunk-fg/80">
        ...
        <span>
          {item.count} unchanged {item.count === 1 ? "row" : "rows"}
        </span>
      </div>
    );
  }
  ...
  if (item.kind === "move") {
    return (
      <div
        className="group flex h-8 items-center gap-2 bg-diff-move-bg px-3 font-mono text-xs"
        title={`moved from row ${(item.row.oldIndex ?? 0) + 1}`}
      >
  ...
  return (
    <div
      className={`group flex h-8 items-center gap-2 px-3 font-mono text-xs transition-opacity ${
        tone === "add" ? "bg-diff-add-bg" : tone === "del" ? "bg-diff-del-bg" : "text-muted-foreground"
      } ${dimmed ? "opacity-50" : ""}`}
    >
      <span
        className={`w-4 shrink-0 text-center text-sm leading-none font-bold ${
          tone === "add" ? "text-diff-add-fg" : tone === "del" ? "text-diff-del-fg" : ""
        }`}
      >
        {tone === "add" ? "+" : tone === "del" ? "−" : ""}
      </span>
      <LineNumbers old={tone === "add" ? null : row.oldIndex} new={tone === "del" ? null : row.newIndex} />
      <Cells
        values={values}
        columns={columns}
        widths={widths}
...
function LineNumbers({ old, new: newIdx }: { old: number | null; new: number | null }) {
  return (
    <span className="flex shrink-0 gap-1.5 font-mono text-[10.5px] leading-none text-foreground/55 select-none">
...
function ChangedCell({ from, to }: { from: string; to: string }) {
  return (
    <div className="flex min-w-0 flex-col gap-0.5 py-1 font-mono text-xs leading-4">
      <span
        className="truncate rounded-sm bg-diff-del-token px-1 text-diff-del-fg line-through decoration-diff-del-fg/50"
        title={from}
      >
        {from || "\u00A0"}
      </span>
...
export function DiffView({
  ...
  const matching = useMemo(() => {
    ...
    return new Set(
      result.rows
        .filter((r) => r.values.some((v) => v.toLowerCase().includes(q)) || (r.key?.toLowerCase().includes(q) ?? false))
        .map((r) => r),
    );
  }, [query, result.rows]);

  const visibleRows = useMemo(
    () => (matching ? [...matching] : changesOnly ? result.rows.filter((r) => r.status !== "unchanged") : result.rows),
  ...
  const unackCount = useMemo(
    () =>
      result.rows.filter((r) => r.status !== "unchanged" && r.status !== "moved" && resolvedRows[r.rowKey] !== true)
        .length,
  ...
  return (
    <div className="overflow-hidden rounded-xl border bg-card">
        ...
        <div className="ml-auto flex flex-wrap items-center justify-end gap-x-3 gap-y-2">
          <div className="relative">
            ...
            <Input
              ...
              className="h-7 w-44 rounded-md pr-12 pl-8 font-sans text-xs"
            />
            {query ? (
          ...
          <div
            ...
            title={query ? "Searching all rows — clear the search to filter to changes" : undefined}
          >
            <Switch
              id="changes-only"
      ...
      {!hasChanges ? (
        ...
        <div className="py-16 text-center text-sm text-muted-foreground">No rows match “{query}”.</div>
      ) : mode === "code" ? (
        <div ref={scrollRef} className="max-h-[calc(100dvh-300px)] min-h-72 overflow-auto">
          ...
          <div className="sticky top-0 z-10 flex h-8 shrink-0 items-center gap-2 border-b bg-card/95 px-3 font-mono text-[10.5px] font-semibold text-muted-foreground backdrop-blur">
            ...
            <span className="flex shrink-0 gap-1.5 leading-none select-none">
              <span className="w-7 text-right font-normal" title="row number in the older snapshot">
                old
              </span>
              <span className="w-7 text-right font-normal" title="row number in the newer snapshot">
                new
              </span>
            </span>
          ...
          <div style={{ height: codeVirtualizer.getTotalSize(), position: "relative" }}>
            {codeVirtualizer.getVirtualItems().map((vr) => {
              ...
              const isChangeLine = item.kind === "sign" && (item.sign === "+" || item.row.status === "removed");
        ...
        <div ref={scrollRef} className="max-h-[calc(100dvh-300px)] min-h-72 overflow-auto">
          <div style={{ minWidth: 90 + result.columns.length * 150 }}>
            ...
            >
              ...
              <div
                className="py-2 text-center font-mono text-[10px] font-medium text-muted-foreground"
                title="Mark as entered / add a note"
              >
                entered?
              </div>
            </div>
