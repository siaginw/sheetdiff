...
beforeAll(async () => {
  ...
  await db
    .insert(spreadsheets)
    .values({ id: "sh1", userId: "u1", googleId: "g1", title: "Q", url: "https://x", createdAt: 1 });
  await db
    .insert(tabs)
  ...
  await db
    .insert(spreadsheets)
    .values({ id: "sh2", userId: "u1", googleId: "g2", title: "Quiet", url: "https://x", createdAt: 1 });
  await db
    .insert(tabs)
    .values({ id: "tq1", spreadsheetId: "sh2", title: "Q", position: 0, tracked: true, keyColumn: 0 });
  await db
    .insert(snapshots)
    .values([
  ...
  await db
    ...
    .values({ snapshotId: "sq1", tabId: "tq1", added: 0, removed: 0, changed: 0, createdAt: 5000 });

  // sh3: snapshots but NO baseline anywhere — the one true "no collection point"
  await db
    .insert(spreadsheets)
    .values({ id: "sh3", userId: "u1", googleId: "g3", title: "NoBase", url: "https://x", createdAt: 1 });
  await db
    .insert(tabs)
    .values({ id: "tnb", spreadsheetId: "sh3", title: "N", position: 0, tracked: true, keyColumn: 0 });
  await db
    .insert(snapshots)
    .values([
  ...
  await db
    .insert(spreadsheets)
    .values({ id: "sh4", userId: "u1", googleId: "g4", title: "Fresh", url: "https://x", createdAt: 1 });
  await db
    .insert(tabs)
    .values({ id: "tf", spreadsheetId: "sh4", title: "F", position: 0, tracked: true, keyColumn: 0 });
  await db.insert(snapshots).values([
    snapRow("sf0", "tf", "rf1", true, 1000, [
      ["Shot", "Qty"],
      ["old1", "1"],
    ]),
    snapRow("sf1", "tf", "rf2", false, 2000, [
      ["Shot", "Qty"],
      ["old1", "1"],
      ["old2", "2"],
    ]),
    snapRow("sf2", "tf", "rf3", false, 3000, [
      ["Shot", "Qty"],
      ["new1", "9"],
      ["old1", "1"],
      ["old2", "2"],
  ...
  await db
    .insert(tabs)
    .values({ id: "td", spreadsheetId: "sh1", title: "D", position: 3, tracked: true, keyColumn: 0 });
  await db.insert(snapshots).values([
    snapRow("sd0", "td", "rd1", true, 1000, [
      ['=HYPERLINK("http://evil.example";"click")', "Qty"],
      ["d1", "5"],
    ]),
    snapRow("sd1", "td", "rd2", false, 5000, [
      ['=HYPERLINK("http://evil.example";"click")', "Qty"],
      ["d1", "5"],
      ["d2", "6"],
    ]),
  ...
  await db
    .insert(snapshotStats)
    .values({ snapshotId: "sd1", tabId: "td", added: 1, removed: 0, changed: 0, createdAt: 5000 });
});
...
describe("entry queue export", () => {
  ...
  it("removed rows are summary-only (nothing to re-key)", async () => {
    ...
    await db
      .insert(snapshotStats)
      .values({ snapshotId: "sa2", tabId: "ta", added: 0, removed: 1, changed: 0, createdAt: 4000 });
    const lines = await csvLines("sh1");
